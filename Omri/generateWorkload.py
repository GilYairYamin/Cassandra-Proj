...
def run_generate_workload():
    ...
    session.execute(
        "CREATE KEYSPACE IF NOT EXISTS simpletry WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : '1' };"
    )

    session.execute("DROP TABLE IF EXISTS person;")

    session.execute(
        "CREATE TABLE IF NOT EXISTS person (id text, name text, toTS MAP<text,timestamp>, PRIMARY KEY (id));"
    )
